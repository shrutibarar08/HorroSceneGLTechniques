...
#include "../core/interface/singleton.h"
...
namespace Engine
{
    namespace Static
    {
        ...
        class EventBus : public ISingleton<EventBus>
        {
        public:
            using ISingleton<EventBus>::Get;
            ...
            )
            {
                ...
                const detail::EventID newID = m_nNextId++;
            ...
            {
                const auto it = m_mapSubscribers.find(token.Type);
...
#define SUB_TO_EVENT(_EventType, _InstancePtr, _Method)                         \
::Engine::Static::EventBus::Get().Subscribe<_EventType>(                    \
(_InstancePtr),                                                         \
EVENT_BIND_EVENT(std::remove_pointer_t<decltype(_InstancePtr)>, _EventType, _Method) \
)

#define UNSUB_FROM_EVENT(_Token) \
::Engine::Static::EventBus::Get().Unsubscribe((_Token))
